...
(function withAngular(angular, navigator) {
  ...
  // var A_DAY_IN_MILLISECONDS = 86400000
    ...
    return [
      ...
      '</span>',
      '</a>',
      '<small class="_720kb-datepicker-clear" ng-click="clearDate()">清除</small>',
      '</div>',
    ...
    , datepickerDirective = function datepickerDirective($window, $document, $compile, $locale, $filter, $interpolate) {
    var linkingFunction = function linkingFunction($scope, element, attr) {
      ...
      var selector = attr.selector
        ...
        , pageDatepickers
        , hours24h = 86400000
        , retouch = attr.dateWarnning === "retouch"
        , setDate = undefined
        ...
        if ($scope.datepickerShow &&
          ...
          showCalendar();
        } else if ($scope.datepickerShow && !$scope.$eval($scope.datepickerShow)) {
      ...
      $scope.clearDate = function clearDate() {
        thisInput.val("");
        thisInput.triggerHandler('input');
      ...
      $scope.$watch('dateRetouch', function (value) {
        if (value) {
          setDate = value.split("+");
        }
      });

      $scope.warnning = !!retouch;
      /*添加日期选择样式*/
      $scope.isTaskDate = function isTaskDate(data) {
        var a = new Date(data);
        if (setDate && retouch) {
          a = $filter('date')(a.getTime(), 'yyyy-MM-dd');
          for (var m = 0; m < setDate.length; m++) {
            var b = $filter('date')(setDate[m], 'yyyy-MM-dd');
            if (a === b) {
              return true;
            }
          }
          ...
          thisInput.on('keyup blur', function onTyping() {
            ...
            if (thisInput[0].value &&
              ...
              try {
                ...
                if (date.getFullYear() && !isNaN(date.getDay()) && !isNaN(date.getMonth()) &&
      ...
      $scope.daysInString = ['日', '一', '二', '三', '四', '五', '六'];//.map(function mappingFunc(el) {
      ...
      if (!$scope.datepickerToggle ||
        $scope.datepickerToggle !== 'false' || !$scope.$eval($scope.datepickerToggle)) {
  ...
  angular.module('720kb.datepicker', [])
    .directive('datepicker', ['$window','$document', '$compile', '$locale', '$filter', '$interpolate', datepickerDirective]);
}(angular, navigator));